...
CCalculate_Uparea::CCalculate_Uparea()
{
    ...
    Parameters.Add_Grid (
        ...
        "Upslope Area: area that flow to a certain pixel",
        PARAMETER_OUTPUT
    );

    Parameters.Add_Table (
        NULL, "PITDATA", _TL ( "Pit data" ),
        "Table per found pit. Coordinate according to the grid in watem.",
        PARAMETER_OUTPUT_OPTIONAL
    );

    Parameters.Add_Value (
        NULL, "PCTOCROP", "Parcel Connectivity to cropland (%)",
        "Percentage of water that will be reduced from a cropland to another cropland.", PARAMETER_TYPE_Double, 70, 0, 100
    );

    Parameters.Add_Value (
        NULL, "PCTOFOREST", "Parcel Connectivity to forest (%)",
        "Percentage of water that will be reduced from a cropland to forest.", PARAMETER_TYPE_Double, 100, 0, 100
    );

    Parameters.Add_Value (
        NULL, "PCTOROAD", "Parcel Connectivity to infrastructure/roads (%)",
        "Percentage of water that will be reduced from a cropland to infrastructure/roads", PARAMETER_TYPE_Double, 70, 0, 100
    );
    ...
    Parameters.Add_Value (
        ...
        "Maximum search radius for flow from pits.",
